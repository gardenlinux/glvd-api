...
For CVEs that are not in the Debian Security Tracker, GLVD will only be able to provide very basic information.
...
Get information about fixed security vulnerabilities in a new minor release.

This endpoint only accepts three digit version numbers as described in https://github.com/gardenlinux/gardenlinux/blob/main/docs/architecture/decisions/0011-garden-linux-versioning.md[ADR 11 Garden Linux versioning].
For the older two digit versioning, see 'Get Patch Release Notes Information (Legacy)' below.
...
=== Get Patch Release Notes Information (Legacy)
